...
import DeveloperInfo from "../developers/DeveloperInfo";
import FavButton from "../FavButton/FavButton";
interface IProps {

}
const DevelopersGames = ({ }: IProps) => {
    useEffect(() => {
        setTimeout(() => {
            setLoading(true);
        });
    ...
    async function getGamesData() {
        setLoading(true);
        const allGames = await fetch(`https://api.rawg.io/api/games?key=${apiKey}&page=${currentPage}&developers=${slug}`, { mode: "cors" })
                        ...
                        <Card shadow="lg" className=" " key={game.id}>
                            ...
                            <CardFooter className={`flex flex-col items-start  space-y-2 `}>
                                ...
                                <div className="flex flex-col space-y-2 md:space-y-0 w-full md:flex-row md:space-x-2 pt-2">
                                    ...
                                    <Button className="w-full" as={Link} radius="sm" onClick={() => router.push(`/${game.slug}`)} showAnchorIcon>about Game</Button>
                                </div>
                            </CardFooter>
                        </Card>
                    )}
